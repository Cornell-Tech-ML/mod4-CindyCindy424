...
def tile(input: Tensor, kernel: Tuple[int, int]) -> Tuple[Tensor, int, int]:
    ...
    input = input.contiguous().view(batch, channel, height, int(width / kw), kw).contiguous().permute(0, 1, 3, 2, 4).contiguous()
    input = input.view(batch, channel, int(width / kw), int(height / kh), kw * kh).contiguous().permute(0, 1, 3, 2, 4).contiguous()
